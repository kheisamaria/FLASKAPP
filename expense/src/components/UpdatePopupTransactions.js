...
const ShowPopupTransactions = ({
	...
	useEffect(() => {
		setUpdateTransactionData({ ...transactionData });
	}, [transactionData]);

	const handleChange = (e) => {
		// Handle changes in the duplicated savingsData
		const { name, value } = e.target;
		setUpdateTransactionData((prevData) => ({
			...prevData,
			[name]: value,
		}));
	};
	...
	const handleConfirm = () => {
		handleUpdateTransaction(updateTransactionData);
					...
					<div className="mt-6">
						<div className="flex flex-row w-full gap-x-6">
							<div className="w-1/2">
								...
								<input
									...
									name="amount"
									value={updateTransactionData.amount}
							...
							<div className="w-1/2">
								...
								<select
									className="h-[50px] w-full border border-gray-300 p-2 mb-4"
									name="payment_method"
									value={updateTransactionData.payment_method}
									onChange={handleChange}
								>
						...
						<div>
							...
							<textarea
								...
								name="description"
								value={updateTransactionData.description}
								onChange={handleChange}
							/>
						</div>
					</div>
