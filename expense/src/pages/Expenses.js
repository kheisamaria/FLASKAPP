...
function Expenses() {
  ...
  const handleUpdateExpenses = (updatedExpensesData) => {
    axios
      .put(`http://localhost:5000/expenses/${updatedExpensesData.expense_id}`, {
        ...updatedExpensesData,
      })
      .then(() => {
        fetchExpenses();
              ...
              <tbody className="h-10 table-body">
                {expenses.map((expense, index) => (
                  <tr key={expense.expense_id}>
                    ...
                    <td>Php {Number(expense.amount).toFixed(2)}</td>
