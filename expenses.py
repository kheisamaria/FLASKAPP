...
def get_expenses():
    query = "SELECT expense_id, user_id, amount, description, frequency, paid FROM expenses"
    result = fetchall(query)
    return result

def get_expense(expense_id):
    query = "SELECT expense_id, user_id, amount, description, frequency, paid FROM expenses WHERE expense_id = %s"
    params = (expense_id,)
    result = fetchone(query, params)
    return result
